...
import json_repair
from components.config import AppConfig, BUDGET_RANGES


def _parse_llm_json(raw: str, provider_name: str) -> dict:
    """
    Parses the JSON report payload an LLM returned, tolerating the kind of
    slip-ups long, creative-writing-heavy generations occasionally produce
    (a dropped comma, a stray control character, an overly long response cut
    short by the token limit) rather than failing the whole report over one
    malformed byte. Tries strict parsing first (fast, and the common case);
    falls back to json_repair's forgiving parser before giving up.
    """
    try:
        ...
        if isinstance(repaired, dict) and repaired:
            return repaired
        raise ValueError(
            f"{provider_name} did not return valid JSON: {strict_error}\n--- raw response ---\n{raw[:2000]}"
        )
...
class AnthropicService:
    ...
    def generate_report_data(self, prompt: str) -> dict:
        ...
        response = client.messages.create(
            model=model_name,
            # A detail-rich suburb (e.g. a well-known, data-heavy one) with a
            # generous budget can push the JSON payload close to 8192 tokens,
            # which truncates the response mid-string and breaks JSON parsing.
            # 16000 leaves real headroom without meaningfully slowing things down.
            max_tokens=16000,
        ...
        return _parse_llm_json(raw, "Claude")
